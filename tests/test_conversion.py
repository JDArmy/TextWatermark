...
def _test_conversion(wm_mode: WMMode, wm_base: int, wm_str: str):
    ...
    if isinstance(wm_str, str):
        assert wm_str == wm_out_str
    else:
        assert str(wm_str) == wm_out_str
...
def test_conversion_restore_error():
    """test conversion restore error"""

    wmc = WMConversion(999, 5)
    with pytest.raises(ValueError):
        wmc.wm_restore_from_arbitrary_base("223101104124")
