...
def _test_template(template, text, wm_final, start_at, confusables_chars_key=''):
    ...
    for wm_base in range(2, confusables_chars_length+1):
        ...
        wmt = WMTemplate(template.CONFUSABLES_CHARS, wm_base,
...
def test_template():
    '''test every template'''
    _test_template(template=homograph_numbers,
                   text='a1a2a3a4a5a6a7a8a9a0a1a2a3a4a5a6a7a89aa0',
                   wm_final='101101',
                   start_at=4)
    _test_template(template=homograph_chinese,
                   text='有八个人抱着白包往北边走',
                   wm_final='101101',
                   start_at=1)
    _test_template(template=homograph_letters,
                   text='Monyer love you',
                   wm_final='101101',
                   start_at=2)
    _test_template(template=homograph_punctuations,
                   text='Mon"yer, love, you.ve_ry-mu\'ch.',
                   wm_final='101101',
                   start_at=1)
    _test_template(template=space_chars,
                   text='有 八 个 人 抱 着 白 包 往 北 边 走有 八 个 人 抱 ',
                   wm_final='101101',
                   start_at=1)

    _test_template(template=whitespace_chars,
                   text='从前有座庙，庙里有个老和尚',
                   wm_final='101101',
                   start_at=1)
    _test_template(template=invisible_chars,
                   text='从前有座庙，庙里有个老和尚',
                   wm_final='101101',
                   start_at=1)
    _test_template(template=html_empty_tags,
                   text='从前有座庙，庙里有个老和尚',
                   wm_final='101101',
                   start_at=1)

    _test_template(template=combining_chars,
                   text='从前有座庙，庙里有个老和尚叫Monyer。',
                   wm_final='101101',
                   start_at=1)

    _test_template(template=font_color,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='101101',
                   start_at=1,
                   confusables_chars_key='black3')
    _test_template(template=font_size,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='101101',
                   start_at=1,
                   confusables_chars_key='110')
    _test_template(template=font_float,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='101101',
                   start_at=1,
                   confusables_chars_key='up3')
    _test_template(template=font_weight,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='101101',
                   start_at=1,
                   confusables_chars_key='100-200')
    _test_template(template=font_style,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='101101',
                   start_at=1,
                   confusables_chars_key='text-decoration-overline')

    _test_template(template=binary_representation,
                   text='从前有座庙，庙里有个老和尚。',
                   wm_final='1000001101',
                   start_at=1,
...
def test_clean_html_tags():
    ...
    wmt = WMTemplate(template.CONFUSABLES_CHARS, 2,
                     template.method)
...
def test_clean_text():
    ...
    # FIND_AND_REPLACE
    _test_clean_text(template=homograph_chinese,
                     text=text+'敖', new_text=text+'敖')
    _test_clean_text(template=homograph_letters,
                     text=text+'AAAAAAA', new_text=text+'ΑАᎪꓮ𝐀𝖠𝗔')
    _test_clean_text(template=homograph_numbers,
                     text=text+'000000', new_text=text+'᱐𝟘𝟎𝟢𝟬𝟶')
    _test_clean_text(template=homograph_punctuations,
                     text=text+'!!', new_text=text+'ǃ！')
    _test_clean_text(template=space_chars,
    ...
    _test_clean_text(template=font_color, text=text, new_text=new_text,
                     confusables_chars_key='black1')

    new_text = f'<span style="display: inline-block;">{text}</span>'
    _test_clean_text(template=font_float, text=text, new_text=new_text,
                     confusables_chars_key='up1')

    new_text = f'<span style="font-size: 100%">{text}</span>'
    _test_clean_text(template=font_size, text=text, new_text=new_text,
                     confusables_chars_key='101')

    new_text = f'<span style=";">{text}</span>'
    _test_clean_text(template=font_style, text=text, new_text=new_text,
                     confusables_chars_key='text-decoration-underline')

    new_text = f'<span style="font-weight: normal;">{text}</span>'
    _test_clean_text(template=font_weight, text=text, new_text=new_text,
    ...
    _test_clean_text(template=binary_representation,
